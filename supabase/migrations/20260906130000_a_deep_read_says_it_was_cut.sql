-- A read cut short by depth says so too.
--
-- `list_team_folder_subtree` stops at sixty-four levels, but reported
-- `truncated` from the folder ceiling alone — so a tree deeper than that
-- dropped everything below it and called the read complete, and the window
-- above handed the scope over as if it were the whole folder. It errs the
-- honest way now: a tree exactly sixty-four deep with nothing under it is
-- called cut, which costs a person one extra sentence and never a missing file.
...
begin
  ...
  with recursive walk as (
    ...
    select drive_file_id, depth,
           row_number() over (order by depth, drive_file_id) as position
    from walk
  ...
  select jsonb_build_object(
    ...
    'truncated', (select count(*) from capped) > p_max_folders
                 or coalesce((select max(depth) from capped), 0) >= 64,
    'items', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'id', files.id,
