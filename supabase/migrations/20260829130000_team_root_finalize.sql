-- Finishing an upload into the space root.
--
-- `service_finalize_uploaded_material` read the destination as a material row
-- and used two things from it: the folder the file must sit in, and the
-- connection the material belongs to. The space root has no such row, so a file
-- uploaded there reached Drive and then could not be committed to the catalog —
-- the last place the root's absence was still assumed.
--
