-- Reading the counts no longer joins the queue.
--
-- Opening the batch window scanned, and the scan wrote a requirement row for
-- every candidate in scope — while the window said, in as many words, that
-- processing starts only after confirmation. True of the processing: nothing
-- ran. But those rows are exactly what other members' devices claim from, so
-- reading the numbers enqueued the work for the whole space, and closing the
-- window left it enqueued.
--
-- `p_commit` splits the two. The window opens with `false` and gets the same
-- numbers from the candidates themselves — each candidate is either a row that
-- already exists or one a commit would create, and the rows a commit would
-- retire are not candidates — and Start runs the committing scan.
