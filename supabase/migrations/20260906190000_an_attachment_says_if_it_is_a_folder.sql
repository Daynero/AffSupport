-- Feature 011: a task's attachment says whether it is a folder.
--
-- A dropped folder is attached as itself — one tile for a landing, rather than
-- fifty for the files inside it. Its category is null, as every folder's is, so
-- the tile called it "File". The kind travels with the row instead, and the
-- tile can say what it is looking at.

CREATE OR REPLACE FUNCTION public.get_team_task(p_team uuid, p_task uuid, p_attachment_cursor bigint DEFAULT NULL::bigint, p_attachment_page_size integer DEFAULT 50)
 RETURNS jsonb
 LANGUAGE plpgsql
