...
const en = {
  ...
  installTitle:'How to install the Agent', install1:'Download and open the DMG.', install2:'Drag “Local Video Compressor Agent” into the Applications folder.', install3:'Open the Terminal app (Applications → Utilities → Terminal).', install4:'Paste this command and press Return: xattr -dr com.apple.quarantine "/Applications/Local Video Compressor Agent.app"', install5:'Open “Local Video Compressor Agent” from the Applications folder.', install6:'It has no Dock icon — look for the film icon in the menu bar at the top-right.', install7:'Your browser opens and connects to the Agent automatically.', install8:'Return to this page and click “I’ve opened the Agent — Connect”.', install9:'To quit later, click the menu bar film icon and choose “Quit Local Video Compressor Agent”.', installFallback:'If the app still will not open, make sure it is in the Applications folder, then run the command above once more.', notNotarized:'The command is needed only because this test build is not yet notarized by Apple. It clears the download-quarantine flag for this one app and does not disable macOS security.',
  ...
  frameRate:'Frame rate', fpsUnit:'fps', frameRateNote:'Caps the output frame rate. Lower values make smaller files; the source rate is never exceeded.',
  qualityCrf:'Quality (CRF)', qualityCrfNote:'Lower is better quality and a larger file. Used when no bitrate is set.', crfOverridden:'Ignored while a target bitrate is set.',
  bitrate:'Target bitrate (kbps)', bitratePlaceholder:'Auto (use quality)', bitrateNote:'Optional. When set, it overrides the quality setting.',
  keepResolution:'Keep original resolution', keepResolutionNote:'On by default. Turn off to downscale to 1080p.', sourceInfo:'Source resolution · frame rate · bitrate',
...
const uk: Record<keyof typeof en, string> = {
  ...
  installTitle:'Як встановити агент', install1:'Завантажте DMG та відкрийте його.', install2:'Перетягніть «Local Video Compressor Agent» у папку Applications.', install3:'Відкрийте застосунок «Термінал» (Applications → Utilities → Terminal).', install4:'Вставте цю команду та натисніть Return: xattr -dr com.apple.quarantine "/Applications/Local Video Compressor Agent.app"', install5:'Відкрийте «Local Video Compressor Agent» з папки Applications.', install6:'Іконки в Dock немає — шукайте іконку плівки в меню-барі вгорі праворуч.', install7:'Браузер відкриється та підключиться до агента автоматично.', install8:'Поверніться на цю сторінку та натисніть «Я відкрив Агент — Підключити».', install9:'Щоб вийти згодом, натисніть іконку плівки в меню-барі та виберіть «Quit Local Video Compressor Agent».', installFallback:'Якщо програма все одно не відкривається, переконайтеся, що вона в папці Applications, і виконайте команду вище ще раз.', notNotarized:'Команда потрібна лише тому, що ця тестова версія ще не нотаризована Apple. Вона знімає позначку карантину завантаження лише для цього застосунку та не вимикає захист macOS.',
  ...
  frameRate:'Частота кадрів', fpsUnit:'кадр/с', frameRateNote:'Обмежує частоту кадрів на виході. Менші значення — менший файл; частота джерела ніколи не перевищується.',
  qualityCrf:'Якість (CRF)', qualityCrfNote:'Менше значення — краща якість і більший файл. Використовується, коли бітрейт не задано.', crfOverridden:'Ігнорується, поки задано цільовий бітрейт.',
  bitrate:'Цільовий бітрейт (kbps)', bitratePlaceholder:'Авто (за якістю)', bitrateNote:'Необовʼязково. Якщо задано, має пріоритет над налаштуванням якості.',
  keepResolution:'Не змінювати роздільність', keepResolutionNote:'Увімкнено за замовчуванням. Вимкніть, щоб зменшити до 1080p.', sourceInfo:'Роздільність джерела · частота кадрів · бітрейт',
