-- Feature 024 — "indexing" is only true while the index is moving.
--
-- A job that is claimed every minute looks alive: it is leased, its lease is current, its row was
-- touched seconds ago. On the beta one such job was claimed 130 times over twelve hours and never
-- saved a checkpoint — every worker ran out of lease on the same page of changes — and for those
-- twelve hours the space's chip said "indexing" with the same two numbers.
--
-- A successful checkpoint resets `attempts`, so the count of claims since the last one is exactly
-- "how long has this made no progress". Past ten the space stops claiming to index and says it
-- needs attention, which is where "check now" lives.
  ...
  if scanning then
    ...
    select exists (
      ...
      where job.connection_id = connection.id
        ...
        -- ...and only while it is getting somewhere. A checkpoint sets `attempts` back to zero,
        -- so a job taken ten times in a row without one is not indexing, it is going round.
        and job.attempts < 10
        and (
