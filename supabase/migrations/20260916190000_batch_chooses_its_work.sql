-- Feature 024 — a batch runs the kinds of work a person chose.
--
-- The batch window counted transcriptions, translations and landing optimizations and started all
-- of them: there was no way to transcribe a folder without also queueing its landings for the
-- whole space. `p_kinds` names the kinds to enqueue; null keeps the old answer (every kind), so the
-- released client is unchanged. Counting (`p_commit = false`) still reports every kind, because the
-- window shows them to choose from. Forward-only; ROLLBACK.md re-applies 20260906100000.

drop function if exists public.scan_library_requirements(uuid, text, uuid[], boolean);

create function public.scan_library_requirements(
  p_team uuid,
...
begin
  ...
  with candidates as (
    ...
    where p_commit and (p_kinds is null or 'transcription' = any(p_kinds))
  ...
  with candidates as (
    ...
    from candidates as candidate
    where p_commit and (p_kinds is null or 'translation' = any(p_kinds))
    on conflict (team_id, source_material_id, source_version, kind, variant) do nothing
    returning 1
  ) select count(*)::integer, (select count(*)::integer from candidates)
  ...
  with candidates as (
    ...
    from candidates as candidate
    where p_commit and (p_kinds is null or 'landing_optimization' = any(p_kinds))
    on conflict (team_id, source_material_id, source_version, kind, variant) do nothing
    returning 1
  ) select count(*)::integer, (select count(*)::integer from candidates)
...
revoke all on function public.scan_library_requirements(uuid, text, uuid[], boolean, text[])
  from public, anon, authenticated;
grant execute on function public.scan_library_requirements(uuid, text, uuid[], boolean, text[])
  to authenticated;
