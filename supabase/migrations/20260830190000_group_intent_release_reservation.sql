-- A move (or any operation finishing through the group-intent path) reserved
-- its target name but never released it on success: the unique reservation
-- index (WHERE reservation_released_at IS NULL) then blocked that name in
-- that folder FOREVER — every later operation on the same name failed with
-- NAME_CONFLICT (found live: two succeeded moves of 16-tail.mp4 held their
-- keys; the compressor's overwrite of that file 409'd at process/start).
-- The commit-mutation and finalize RPCs already release on success; this
-- brings the group-intent completion in line.
...
begin
  ...
  update public.team_operations
     set state = 'succeeded', stage = 'completed', progress = 100,
         reservation_released_at = coalesce(team_operations.reservation_released_at, clock_timestamp()),
