...
_GATE: evaluated before Phase 0 and re-evaluated after Phase 1. Both passes below._
...
| Two type-check configurations with different module resolution | The test tree imports three mutually incompatible worlds; no single Node-style configuration spans them.                                                                                                                            | One unified config was tried and cannot work. This does **not** weaken the repository's explicit-extension rule, which is enforced on _source_ by three existing configs that are untouched — a comment block at the head of the new config states this so it cannot be misread later. |
