...
- [ ] T001 Confirm the D1 gate on this branch — `git diff --name-only v1.1.1..HEAD -- apps/agent packaging packages/shared/src packages/shared/package.json config/production.env` prints nothing — and take the next free migration timestamp from `ls supabase/migrations | tail -2` (after `20260915010000`)
- [ ] T002 Add `[functions.catalog-updater]` with `verify_jwt = false` to `supabase/config.toml`, next to `[functions.catalog-sync]`
