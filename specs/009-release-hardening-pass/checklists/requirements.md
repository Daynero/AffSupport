...
1. _Implementation detail leak_ — the first draft named concrete technologies in requirements
...
2. _Unmeasurable success criteria_ — "faster", "smaller", "more secure" replaced with
...
3. _Unbounded scope_ — the Assumptions section now states explicitly that this feature adds no
...
4. _Evidence separated from specification_ — the audit that motivated each requirement was moved
...
5. _The user's own headline symptom had no requirement._ The audit's D10 (seven separate live
...
6. _Unachievable criteria._ "Output identical to a job that was never stopped" is not guaranteed
...
7. _Criteria that could never fail._ SC-005's ten-minute ceiling sat above a measured 53 seconds,
   ...
   reacting.
8. _Contradictions._ FR-026's "current session" forbade the queue restoration FR-006 requires;
...
9. _Findings with no requirement._ Six security observations — command construction from
...
10. _An architectural finding silently declined._ The audit calls the five separate run
...
11. _A factually wrong finding._ The audit cited a stacking value of 10001 as sitting above every
...
12. _Confidence was not carried forward._ The audit's most quoted conclusion — that a brief
...
13. _Two requirements had zero tasks._ **FR-004** (a stop releases the queue slot immediately, so
    ...
    T045a and T020a.
14. _A success criterion that could not be demonstrated._ **SC-009** requires a sweep of
...
15. _Ordering that buried the confirmed-live holes._ The plan put four security fixes in wave
...
16. _Countable claims left as prose._ SC-008's "at least thirty attack attempts" and SC-023's
...
17. _An unbounded task on the critical path._ "Fix the Windows failures the first run surfaces"
    is unknowable scope sitting on the merge gate. Split into T119 (discovery, produces a list),
    T119a and T119b (consume it).
18. _A judgement call left undefined under delivery pressure._ "Flip the path ledger to enforce
    after one beta cycle" had no exit criterion. T178 now fixes the threshold — 200 observed uses
    across 10 sessions with zero unexplained refusals — and requires it to be set _before_ T176
    ships.
19. _A repository setting with no owner._ SC-006 needs the four checks to be **required** to
    merge; the workflows are files, the requirement is a settings change. Added T118a.
20. _A type-safety hole in the new contract._ The lifecycle registry was typed
...
21. _Success criteria that restated their requirement._ Five (SC-003, SC-007, SC-011, SC-015,
    ...
    they are.
22. _Two internal contradictions found while fixing the above._ The data model required every
...
23. _Story-independence claims that were not true._ User Story 2's T093 needs User Story 1's
...
- _Seven user stories instead of the usual three._ The request spans six independent outcomes
...
- _71 functional requirements and 23 success criteria._ This is a hardening pass over an entire application rather than
  one capability; the count reflects surface area, not scope creep. They are grouped by theme so
  a reader can navigate them.
- _A `Dependencies` section was added._ Four requirements (FR-017, FR-028, and the platform half
