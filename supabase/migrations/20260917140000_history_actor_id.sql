-- Feature 024 — the history can be narrowed to what I did.
--
-- "Mine" needs to know who did each thing, and the list carried only the actor's name snapshot.
-- It now returns actor_id too. The return type grows a column, so the function is dropped and
-- made again. ROLLBACK.md re-applies 20260917130000's definition.

drop function if exists public.list_team_audit_events(uuid, integer, timestamptz);

create function public.list_team_audit_events(
  p_team uuid,
