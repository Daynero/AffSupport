-- A preview pass that stopped must stop claiming progress too.
--
-- 20260829100000 fixed this for `indexing`: a scan nobody was working on kept
-- the chip spinning forever, so the catalog now has to prove a job is alive.
-- `preparing` was left with the same flaw and the owner hit it — "Готуємо
-- превʼю · 480 з 481", spinning for as long as the space stayed open, one
-- preview short of done and no way to tell whether anything was happening.
--
-- Nothing in that branch required movement. A single thumbnail that never
-- resolves is enough: the warm pass re-claims it every ten minutes, or a
-- provider stop code defers it every pass, and either way the count never
-- changes while the chip promises work.
--
-- The bound is the one this project already established in 20260902130000:
-- Drive produces a thumbnail within minutes of an upload or never, so after a
-- day there is nothing to wait for. A preview older than that is not being
-- prepared — the file simply has no thumbnail, which its tile already says
-- plainly. The same applies to a landing render nobody has touched in a day.
--
