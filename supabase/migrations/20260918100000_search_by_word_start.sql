-- Feature 024 — a search finds a file by the start of its name.
--
-- The query was matched as whole words: "horses" found nothing in a folder of Horses1.png …
-- Horses10.png, because the name's words are "horses1" and "horses10". The palette and the
-- explorer's search are typed into a letter at a time and expect what Drive does — every word
-- of the query matches the start of a word. `private.material_search_query` splits the query
-- the way the index splits names and asks for each word as a prefix; a query written with
-- web-search syntax (quotes, "or", a leading minus) keeps that meaning.
-- Forward-only; ROLLBACK.md re-applies 20260916230000's search.

create or replace function private.material_search_query(p_query text)
returns tsquery
language sql
immutable
set search_path = ''
as $$
  select case
    when p_query ~* '"|(^|\s)-|\sor\s' then pg_catalog.websearch_to_tsquery('simple'::regconfig, p_query)
    else (
      select pg_catalog.to_tsquery(
        'simple'::regconfig,
        coalesce(
          pg_catalog.string_agg(
            -- Quoted as a tsquery lexeme ('it''s'), not as SQL text: quote_literal writes E'…'.
            ''''
              || pg_catalog.replace(pg_catalog.replace(word.lexeme, '\', '\\'), '''', '''''')
              || ''':*',
      ...
      from pg_catalog.unnest(pg_catalog.to_tsvector('simple'::regconfig, p_query)) as word
    )
  end
$$;

revoke all on function private.material_search_query(text) from public;
