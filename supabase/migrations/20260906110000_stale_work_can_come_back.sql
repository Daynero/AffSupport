-- Work that becomes work again comes back.
--
-- A requirement retired to `stale` keeps its row, and the row keeps the unique
-- key. So when the same job was warranted again — a video that had no
-- transcript gets one, and its translation becomes real work — the insert hit
-- `on conflict do nothing` and silently dropped it. The batch window counted
-- the candidate and offered it; the claim loop found no pending row and
-- reported the batch complete a second after Start. Two numbers, one press
-- apart, and no way for a person to tell which was lying.
--
-- A stale row is revived instead. Anything further along — pending, leased,
-- running, ready — is left exactly as it is.
...
begin
  ...
  with candidates as (
    ...
    where public.team_library_requirements.state = 'stale'
    returning 1
