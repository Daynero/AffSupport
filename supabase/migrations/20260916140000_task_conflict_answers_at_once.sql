-- A task edited from a stale copy is refused at once, not after a minute.
--
-- update_team_task refused a stale `expectedUpdatedAt` with SQLSTATE 40001 (serialization failure).
-- PostgREST retries a transaction that fails that way, so the request hung until the gateway gave up
-- with a 504 about a minute later — and the card's progress scale, waiting on it, showed a value that
-- was never saved and then snapped back. The same refusal, raised as P0001, reaches the client
-- straight away as SOURCE_CHANGED. The body is 20260905160000's otherwise unchanged.
-- Forward-only; ROLLBACK.md re-applies 20260905160000's definition.
...
begin
  ...
  if p_patch ? 'expectedUpdatedAt'
    ...
    raise exception 'SOURCE_CHANGED' using errcode = 'P0001';
  end if;
