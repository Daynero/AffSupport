...
describe('safe output paths', () => {
  it('supports spaces and Cyrillic and does not overwrite', async () => {
    ...
    const input = path.join(temp, 'Моє відео (тест).mov'); await writeFile(input, 'source');
  ...
  it('detects application-created compressed suffixes', () => { expect(appearsCompressed('/tmp/video_compressed.mp4')).toBe(true); expect(appearsCompressed('/tmp/video_compressed_3.mp4')).toBe(true); expect(appearsCompressed('/tmp/video.mp4')).toBe(false); });
});
