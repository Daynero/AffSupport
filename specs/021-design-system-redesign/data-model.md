...
_Colour_ — seven roles × the shades a component needs:
...
_Text_ — one ramp, each step with a role: `display` (page title), `title` (panel heading),
...
_Space_ — `1`–`6` as today (4/8/12/16/20/24px), plus `0` and `px`.

_Radius_ — `sm` (6), `md` (10), `lg` (14), `xl` (18), `full` (999). The nesting rule from
...
_Shadow_ — `sm`, `md`, `lg`, `menu`. Used sparingly; the product separates surfaces by tint,
not by elevation.

_Motion_ — `fast` (150ms), `base` (220ms), `slow` (300ms, ceiling); `ease-out`,
`ease-in-out`, `linear`. Under `prefers-reduced-motion: reduce` all three durations resolve
to `1ms`.

_Layer_ — the existing `--layer-*` set, unchanged in values, documented in one place.
