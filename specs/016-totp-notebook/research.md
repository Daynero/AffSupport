...
- _A plain `secret text` column with an RLS policy._ Fewer moving parts, and RLS
...
- _End-to-end encryption under a user passphrase._ Strictly stronger — the
...
- _Never send the seed to the browser; compute codes in an Edge Function._
...
_Synchronous_ — see D3; an `await` before the clipboard write is a real
cross-browser hazard.

_In `shared`_ — it is domain logic, which principle I puts in
...
_Dependency-free_ — `packages/shared/package.json` has no runtime dependencies
...
- _`crypto.subtle.importKey` + `sign('HMAC')`._ The obvious choice, and it is
...
- _An npm package (`otpauth`, `otplib`)._ Battle-tested and less code to own.
...
- _`new ClipboardItem({'text/plain': promise})`._ The standard escape hatch for
  ...
  fallback anyway.
- _Fetch the seed on press, then write._ One round trip inside the gesture, the
  exact hazard above. Rejected.
- _Prefetch each seed on row hover._ Would preserve a seed-per-request model,
...
- _List names only; `reveal_two_factor_secret(p_entry)` per row._ A smaller
...
- _Add `twoFactor: {}` to `WEB_TOOL_REQUIREMENTS`._ Rejected: an empty
  requirement still changes the signed-manifest comparison, so it blocks
  deployment for a tool with nothing to gate.
- _Keep the notebook off the registry and hand-route it._ Rejected: the registry
...
- _An NTP or time-API call._ A new external origin, a CSP entry, and a
  third-party dependency for a warning line. Rejected.
- _Read the `Date` header off the Supabase RPC response._ Free, but
...
- _Say nothing and let codes fail._ Rejected — it is the spec's own edge case.
