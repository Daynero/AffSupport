...
Рекурсивний обхід підключеної папки виконує обмежений воркер, який **кожні 10 секунд** запускає
...
Без готових задач HTTP-виклик пропускається. Одночасно допускається до трьох
воркерів; кожен бере одну задачу та читає сторінки в межах бюджету (типово 8 секунд).
Кожна сторінка зберігає прогрес без звільнення lease; наприкінці проходу воркер
явно повертає задачу в чергу. Відхилений checkpoint зупиняє прохід. Звичайна
перевірка змін після завершення індексації залишається раз на хвилину.
