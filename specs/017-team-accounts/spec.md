...
| An agent carries any number of _runs_ (`team_agent_runs`); "free" = no runs                                                                                                                | The first model was one note per agent; in use an agent carries several launches at once (owner: "тут може бути не один запуск"). Runs are rows with their own pencil, bin and plus; the note column was migrated into first runs and dropped.                                                                                                                               |
...
| A section's heading is the heading alone — no eyebrow repeating it                                                                                                                                                                                  | "ЗАВДАННЯ / Завдання" and "АКАУНТИ / Акаунти" said the same word twice; the same repetition went from the task editor, the media picker and the two processing dialogs.                                                                                                                                                                                                                                                                                 |
| A task carries its own date (`task_date`), shown on the card and settable in the editor; null means the day it was created                                                                                                                          | The board reads by date, and the date that matters is the one the work is for — a launch dated the day it goes live — not the moment the row was made. `created_at` stays untouched: it is the audit record and the key the list pages by. Clearing the date is a first-class action ("Day it was created"), so there is no value a person cannot take back.                                                                                            |
| The board filters, orders and pages by that date — the caller sends its range as instants _and_ as plain days                                                                                                                                       | Showing one date and filtering by another is the bug the date itself created: a task moved to the 18th still answered "Today". `created_at` is an instant and `task_date` is a calendar day, and only the browser knows the timezone that joins them, so it sends both forms of one range and each row is judged by the form that fits it. Nothing is guessed in SQL. "This month" now spans the whole month, since a task can be dated ahead of today. |
| The tasks board has one "Unfold all / Fold all" control beside "Create"                                                                                                                                                                             | Reading every brief in a column is how a morning starts; the label says which way the next press goes. Card fold state therefore lives in the list, not in each card.                                                                                                                                                                                                                                                                                   |
| Every control in the filter row shares one size, weight, height and radius, and one selected look: accent border, accent tint, accent text                                                                                                          | The row was three families — 0.85rem date pills, a 10px-radius icon button, and status options inheriting the panel's 1.3rem body text — and two different "selected" treatments, one of which (dark `--color-accent-active` on a dark panel, 2.9:1) was barely legible. The accounts filter pills follow the same rule.                                                                                                                                |
...
- Migrations: `20260905160000_team_task_date.sql` (a task's own date), `20260905180000_team_task_date_filter.sql` (the list reads by it).
...
## Part 4 — a run can be marked

The runs on an agent are read down a column, and the one thing the owner could
not say about a run was how it is going. A marker is that, in the one gesture a
list like this can afford: press the run and it cycles — unmarked, green,
amber, red, unmarked. Three colours, nothing to learn, and four presses always
put the run back where it started, which is what makes marking safe to try.

| Decision                                                         | Why                                                                                                                                                  |
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| The marker lives on the run, not on the agent                    | An agent carries several runs and they are rarely going the same way.                                                                                |
| The product never says what green means                          | The team's own reading is the only one that would be right; the colours are named by what they look like — Green, Amber, Red.                        |
| A press cycles rather than opening a menu                        | One gesture for the whole column, and the cycle returns: there is nothing to undo.                                                                   |
| The press target is the whole run, as a layer under its contents | The pencil, the bin and the tasks link inside it are buttons and links of their own; a button cannot hold a button, so the target is behind.         |
| The marker chips count agents, not runs                          | A filter chip can only promise how many rows pressing it leaves on screen.                                                                           |
| A colour drops the accounts that cannot answer it                | The same rule the occupancy filter already follows.                                                                                                  |
| An unknown colour parses as no marker                            | A build that does not know a colour can still show the run.                                                                                          |
| The colours live behind one trigger, not as four chips           | They are the filter reached for least, and four more chips pushed the fold onto a line of its own; the closed trigger says how many runs are marked. |
| "Clear all markers" lives in that menu, with an undo             | It is the only other thing on the screen about markers, and one press over a lot of marking gets the undo a release gets.                            |

### Contracts (part 4)

- Migration: `supabase/migrations/20260906150000_a_run_can_be_marked.sql` —
  `team_agent_runs.marker` (`green` / `amber` / `red` / null),
  `private.team_agent_runs_json` carries it, and it joins the realtime publication.
- RPC: `set_team_agent_run_marker(p_team, p_run, p_marker)` — returns the whole
  agent, like every other run call; a colour it does not know is `INVALID_INPUT`.
- RPC: `clear_team_agent_run_markers(p_team)` — takes every marker off the space
  in one statement and returns `[{ run_id, marker }]`, which is what the toast's
  undo writes back.
- Shared: `TeamAgentRunMarker`, `nextTeamAgentRunMarker`, `teamAgentHasMarker`,
  `TeamAccountMarkerFilter`; `TeamAgentRun.marker`; `filterTeamAccounts({ marker })`;
  `TeamAccountCounts.markers`.
- Web: the run is a press target in `AgentRow`; `MarkerFilter.tsx` is the
  trigger, its menu and the clear; `useAccounts.setRunMarker` / `clearMarkers`
  write.
- Shared: `parseTeamAgentRunMarkerSnapshots` reads what a clear returned.
- Tests: `tests/team-accounts-sql.test.ts` (both RPCs and their guards),
  `tests/team-accounts-contract.test.ts` (the cycle, the parse, the filter),
  `tests/team-accounts.test.tsx` (four presses, the chips, a viewer).

### The row at every width

The agent row is a grid whose fixed tracks — id, status, tasks, the row's own
buttons — are spent before the runs get a pixel, and a run line is a flex row of
things that do not shrink: the note, the day it was written, a pencil, a bin and
"+ Run". When the two stop fitting, nothing throws and the page does not scroll
sideways: `overflow-wrap: anywhere` folds the note into a column of single
letters inside its own cell. So the invariant is not "the page does not
overflow" but

> every run line's `scrollWidth` equals the width of the cell holding it — and
> above 1000px every row keeps the same height.

`npm run check:accounts-layout` measures it in a real browser against the
running beta, at the widths either side of every breakpoint — 560, 1000 and
1200 — plus the phones nobody remembers (320, 360, 390, 414, 480, 561, 700, 900,
1001, 1100, 1201, 1280, 1440, 1600). It writes both halves of the worst case
into the page first: the example run the product itself offers, because the
failure it caught at 1001 depended on the note's length, and a day, because a
run written today prints none and the day is the widest thing on the line after
the note. Add both sides of any breakpoint you add. Run it after putting another
action in the row or a longer word on a button; jsdom has no layout, so the test
suite cannot.

Three layouts, and the reason for each: a table above 1000; below it the row
stacks, because six columns fit only by taking the words off "Make free" and
"+ Run", and a nameless circled tick beside a nameless plus is the guessing game
this rework was undoing; below 560 the agent's buttons take a line of their own
and the run line wraps, because a phone's 300px strip cannot hold 250px of
controls beside a note.
