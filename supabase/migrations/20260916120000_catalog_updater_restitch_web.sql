-- Feature 023 — re-stitched videos behind the catalogs, prepared by an open Soty tab.
--
-- With re-stitching on, a member's open Soty tab on a computer with the Soty app keeps a spare
-- re-stitched copy of every catalog's video ready next to it — the released app already re-stitches
-- a team video and uploads the result through the ordinary process grants. At each round the worker
-- points the sheet's video column at the spare, the spare becomes the copy in use, the copy used
-- before is deleted for good, and a new spare is asked for. Only one spare per catalog waits; turning
-- re-stitching off or stopping deletes the spares and leaves each sheet on the copy it points at. With
-- no tab open, rounds still move the IDs and keep the video they point at.
--
-- What this adds:
--   team_catalog_restitch_copies     spare / in-use / retired copies per catalog
--   private.catalog_restitch_jobs    one per catalog that needs a spare; leased by a member's tab
...
--   save_team_catalog_updater accepts restitch; claim / bind / heartbeat / complete for drive-ops,
--   the finalize hook, the round swap, and the deletion queue for retired copies
--
-- Additive for the released desktop app. Forward-only; reverse steps in ROLLBACK.md.
...
create table private.catalog_restitch_jobs (
  ...
  -- The member whose tab holds the lease; one lease at a time per member and space.
...
-- ---------------------------------------------------------------------------------------------
-- The updater's state, now with the spares.
...
-- ---------------------------------------------------------------------------------------------
-- A member's tab (through drive-ops, as that member).

/*
 * Leases the next spare to prepare for a member who may process in the space. Returns null when there
 * is nothing to do, or the job with everything drive-ops needs to start the process as that member.
 */
...
begin
  ...
  -- One at a time per member: their own lease still running means their computer is still working.
  if exists (
    select 1 from private.catalog_restitch_jobs as job
    where job.team_id = p_team and job.lease_owner = p_actor and job.state = 'leased'
  ...
  select jsonb_build_object(
    ...
    -- Earlier attempts whose leases lapsed may have left processes open, holding the copy's name.
    'openOperationIds', coalesce((
      select jsonb_agg(started.operation_id)
      from private.catalog_restitch_operations as started
      join public.team_operations as operation on operation.id = started.operation_id
      where started.catalog_material_id = claimed.catalog_material_id
        and operation.state in ('pending', 'running')
    ), '[]'::jsonb),
    'videoMaterialId', video.id,
    'videoName', video.name,
    'videoDriveVersion', video.drive_version,
    'destinationFolderId', folder.id,
    'updateCount', record.update_count,
    -- In the shapes the app's restitch delegate reads (shared `parseTeamRestitchDefaults` and
...
/*
 * The tab's report. A failure backs the job off (1, 5, 15 minutes). Success itself is recorded by
 * the finalize hook; a job still leased after a reported success had its output refused, so it is
 * queued again.
 */
...
begin
  ...
  where candidate.catalog_material_id = p_job
    and candidate.lease_owner = p_actor
...
revoke all on function public.service_claim_restitch_job(uuid, uuid, bytea, integer)
  from public, anon, authenticated;
revoke all on function public.service_bind_restitch_job_operation(uuid, bytea, uuid)
  from public, anon, authenticated;
revoke all on function public.service_heartbeat_restitch_job(uuid, uuid, bytea, integer)
  from public, anon, authenticated;
revoke all on function public.service_complete_restitch_job(uuid, uuid, bytea, text, text)
  from public, anon, authenticated;
...
grant execute on function public.service_claim_restitch_job(uuid, uuid, bytea, integer)
  to service_role;
grant execute on function public.service_bind_restitch_job_operation(uuid, bytea, uuid)
  to service_role;
grant execute on function public.service_heartbeat_restitch_job(uuid, uuid, bytea, integer)
  to service_role;
grant execute on function public.service_complete_restitch_job(uuid, uuid, bytea, text, text)
  to service_role;
