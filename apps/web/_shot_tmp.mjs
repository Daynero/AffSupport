...
await page.evaluate(() => {
  document.querySelectorAll('.public-home').forEach(el => {
...
async function hoverCount(x, y) {
  ...
  return await page.evaluate(() => {
    let n = 0;
    document.querySelectorAll('.soty-cell').forEach(e => {
      if (e.style.transform) n++;
    });
    return n;
  });
}
...
await page.screenshot({
  path: O + '/lift-check.png',
  clip: { x: 0, y: 300, width: 340, height: 420 }
});
await browser.close();
