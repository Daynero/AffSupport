...
describe('the old sections, as aliases (011)', () => {
  it('opens /files, /landings, /creatives, /settings and /trash inside the explorer', () => {
    ...
    expect(parseTeamRoute('/team/space-1/landings')).toMatchObject({ query: { scope: 'space' } });
    expect(parseTeamRoute('/team/space-1/creatives')).toMatchObject({ query: { scope: 'space' } });
    expect(parseTeamRoute('/team/space-1/files')).toMatchObject({ query: { scope: 'folder' } });
