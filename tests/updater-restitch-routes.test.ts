...
} from '../supabase/functions/drive-ops/updater-restitch.js';

/**
 * Feature 023: the routes a member's open tab calls in drive-ops to prepare the updater's spare
 * copies, with the database, process start and Drive replaced by recording fakes.
 */

const TEAM = '23500000-0000-4000-8000-000000000001';
const JOB = '23500000-0000-4000-8000-000000000002';
const ACTOR = 'actor';
...
function setup(overrides: { claim?: unknown; start?: () => Promise<never>; bound?: boolean } = {}) {
  ...
  const deps: UpdaterRestitchDeps = {
    rpc,
    startProcess: vi.fn(
      overrides.start ??
...
describe('claiming', () => {
  it('refuses a malformed space before touching the database', async () => {
    const { deps, rpc } = setup();
    await expect(claimRestitchJob(deps, ACTOR, { teamId: 'x' })).rejects.toThrow(/INVALID_INPUT/);
    expect(rpc).not.toHaveBeenCalled();
  });

  it('starts the process as the member and hands back what the app needs', async () => {
    const { deps, call } = setup();
    const result = await claimRestitchJob(deps, ACTOR, { teamId: TEAM });
    expect(call('service_claim_restitch_job')).toEqual({
      p_team: TEAM,
      p_actor: ACTOR,
  ...
  it('closes the operations lapsed leases left open before starting again', async () => {
    ...
    await claimRestitchJob(deps, ACTOR, { teamId: TEAM });
...
describe('heartbeat and completion', () => {
  it('renews with the hashed lease, and refuses a malformed one', async () => {
    const { deps, call } = setup();
    expect(await heartbeatRestitchJob(deps, ACTOR, { jobId: JOB, leaseToken: LEASE })).toEqual({
      cancel: false
    });
  ...
  it('passes the outcome with a clean error code', async () => {
    const { deps, call } = setup();
    await completeRestitchJob(deps, ACTOR, {
      jobId: JOB,
      leaseToken: LEASE,
      outcome: 'failed',
    ...
    await expect(
      completeRestitchJob(deps, ACTOR, { jobId: JOB, leaseToken: LEASE, outcome: 'maybe' })
    ).rejects.toThrow(/INVALID_INPUT/);
  });
});
