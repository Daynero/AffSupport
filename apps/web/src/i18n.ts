...
const en = {
  ...
  install1:'Download the Mac Agent.', install2:'Open the downloaded ZIP.', install3:'Move Local Video Compressor Agent to Applications.', install4:'Control-click the app and choose Open.', install5:'Confirm Open.', install6:'Return to this page.', notNotarized:'This warning appears because the test version is not yet notarized by Apple.',
...
const uk: Record<keyof typeof en, string> = {
  ...
  install1:'Завантажте Mac Agent.', install2:'Відкрийте завантажений ZIP.', install3:'Перемістіть Local Video Compressor Agent до Applications.', install4:'Натисніть застосунок із Control і виберіть Open.', install5:'Підтвердьте Open.', install6:'Поверніться на цю сторінку.', notNotarized:'Це попередження з’являється, бо тестову версію ще не нотаризовано Apple.',
