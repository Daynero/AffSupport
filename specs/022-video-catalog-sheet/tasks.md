...
- [x] T042 **Owner-confirmed only** — rollout per quickstart §8: fast-forward `beta`, `release:backend-plan`, packaged beta verification of the exact SHA, `release:backend-apply`, `deploy:web`; then merge into `main` — Done 2026-09-15 at `c07ad54`: `beta` fast-forwarded, packaged beta verified, migration `20260915010000` and the 10 functions applied, `deploy:web` completed (backend apply and web deploy run by another agent with the owner's permission). Verified from outside: the live bundle carries the catalog, both new RPCs exist and refuse anon, `stable.json` still 1.1.1. The agent's local page (Safari) keeps the 1.1.1 web until the next desktop release — accepted by the owner. Merging into `main` is a separate task
