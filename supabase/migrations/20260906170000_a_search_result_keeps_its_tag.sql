-- Feature 011: the search carries a file's tag too.
--
-- A tag is meant to be read wherever the file is, and the explorer's search is
-- the same files in another view. Without this, colouring a file and then
-- searching for it made the colour vanish, which reads as "the tag was lost".
--
-- Setting one is still the owner's alone (`set_team_material_tag`); this only
