...
0. `20260829120000_team_root_reservation.sql`: restore the previous body of
...
1. `20260829110000_team_root_destination.sql`: drop
...
2. `20260829100000_team_storage_health_stall.sql`: re-apply the previous body of
   `public.get_team_storage_health` from `20260827109000`. Read-only.
3. `20260827109000_team_storage_health.sql`: drop `public.get_team_storage_health`. Read-only;
   nothing depends on it but the storage chip.
4. `20260827108000_team_search_scope.sql`: drop
...
5. `20260827107000_preview_warm_schedule.sql`: `select cron.unschedule(jobid) from cron.job
...
6. `20260827106000_team_thumbnail_session.sql`: drop `public.assert_team_member`; delete
...
7. `20260827105000_team_catalog_page_markers.sql`: drop
...
8. `20260827104000_team_folder_tree_and_paging.sql`: drop `public.list_team_folder_page`,
   `public.list_team_folder_tree`, `public.team_material_kind`. Nothing else depends on them.
9. `20260827103000_team_material_index_columns.sql`: drop `public.remove_team_drive_selection`;
...
10. `20260827102000_team_connection_state_011.sql`: drop `public.service_record_access_expiry`,
...
11. `20260827101000_team_drive_selections.sql`: drop `public.service_list_connection_selections`,
...
12. `20260827100000_team_error_codes_011.sql`: delete the six 011 rows from
...
13. `20260823120000_team_ux_lifecycle.sql`: revoke and drop
    ...
    if that matters.
14. `20260815117000_request_catalog_resync.sql`: revoke and drop
...
15. `20260814102000_creative_library_security.sql`: first remove only
...
16. `20260814101000_creative_library_actions.sql`: stop local Library workers and let their
...
17. `20260814100000_creative_library_foundation.sql`: export task, contribution, processing
...
18. `20260810113000_team_landing_render_delivery.sql` (feature 004 delivery helpers): revoke and
...
19. `20260810090000_team_landing_renders.sql` (feature 004 — reverse after delivery helpers): revoke and
...
20. `20260802100000_team_direct_member_testing.sql`: revoke and drop
...
21. `20260801103000_team_analytics.sql`: revoke/drop
...
22. `20260801102000_team_transfer_operations.sql`: revoke/drop `get_operation`,
...
23. `20260801101500_team_preview.sql`: revoke/drop `get_material_preview`,
...
24. `20260801101000_team_catalog_search.sql`: unschedule only the named
...
25. `20260801100000_team_membership_actions.sql`: revoke/drop `list_team_members`,
...
26. `20260801095000_team_invitation_drive_actions.sql`: drop the caller RPCs
...
27. `20260801094000_team_security_foundation.sql`: remove `team_operations` and
...
28. `20260801093000_team_operations_audit.sql`: drop `public.team_catalog_events`,
...
29. `20260801092000_drive_vault_catalog.sql`: detach test connections first; delete their
...
30. `20260801091000_teams_members_invitations.sql`: drop
    `public.team_invitations`, `public.team_members`, and `public.teams`. Keep the `citext`
    extension if another feature uses it.
31. `20260801090000_team_contract_seed.sql`: drop `public.language_options`,
