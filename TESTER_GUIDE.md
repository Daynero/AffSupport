...
The Agent has **no Dock icon** — it runs from the menu bar (the film icon at the top-right). On first launch it opens the website and connects automatically. Choose **Select videos**, then **Start compression**. Results are saved beside each original unless you choose another folder. Videos never leave your Mac.

## Quit and report a problem
Quit the app from its **menu bar icon** → **Quit Local Video Compressor Agent** (there is no Dock icon). On the website choose **Copy diagnostics** and send that text with a short description. The report excludes videos and full private paths.
