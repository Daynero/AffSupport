-- Admins can manage the connected storage and must be able to request the same
-- idempotent full scan as the owner. This matches the storage-management role
-- checks used elsewhere in the workspace.
...
begin
  ...
  select connection.id into resolved_connection
  ...
  perform private.record_team_audit(p_team, actor, 'drive.resynced',
    jsonb_build_object('connection_id', resolved_connection, 'state', 'scanning'), 'succeeded', null);
