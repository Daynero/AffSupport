...
const en = {
  ...
  installTitle:'How to install the Agent', install1:'Download and open the DMG.', install2:'Drag Local Video Compressor Agent to Applications.', install3:'Open the app from Applications.', install4:'If macOS blocks the app, click Done. Do not move the app to Trash.', install5:'Open System Settings → Privacy & Security.', install6:'Scroll down to the Security section.', install7:'Find the message about Local Video Compressor Agent and click Open Anyway.', install8:'Confirm with Touch ID or your password, then click Open.', install9:'Return to this page and click “I opened the Agent — Connect”.', installFallback:'If Open Anyway is not available, open Applications in Finder, Control-click Local Video Compressor Agent, and choose Open.', notNotarized:'This warning appears because the test version has not yet been notarized by Apple. You do not need to disable macOS security.',
...
const uk: Record<keyof typeof en, string> = {
  ...
  installTitle:'Як встановити агент', install1:'Завантажте DMG та відкрийте його.', install2:'Перетягніть Local Video Compressor Agent у папку Applications.', install3:'Відкрийте програму з папки Applications.', install4:'Якщо macOS заблокує запуск, натисніть «Готово». Не переміщуйте програму в Смітник.', install5:'Відкрийте «Системні параметри» → «Конфіденційність і безпека».', install6:'Прокрутіть до розділу «Безпека».', install7:'Біля повідомлення про Local Video Compressor Agent натисніть «Відкрити однаково».', install8:'Підтвердьте дію через Touch ID або пароль і натисніть «Відкрити».', install9:'Поверніться на цю сторінку та натисніть «Я відкрив Агент — Підключити».', installFallback:'Якщо кнопки «Відкрити однаково» немає, відкрийте папку Applications у Finder, затисніть Control, натисніть на Local Video Compressor Agent і виберіть «Відкрити».', notNotarized:'Це попередження з’являється, тому що тестова версія ще не нотаризована Apple. Вимикати захист macOS не потрібно.',
