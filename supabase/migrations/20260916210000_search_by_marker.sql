-- Feature 024 — search by a file's colour mark.
--
-- A colour is set on a file to find it again; the search could not find by it. `search_materials`
-- accepts a `marker` filter (the seven tag colours), and the vocabulary says which colours the
-- space's files carry, so the filter offers only those. Forward-only; ROLLBACK.md re-applies
-- 20260906170000's search and 20260916200000's vocabulary.
...
begin
  ...
  select pg_catalog.jsonb_build_object(
    ...
    'usedMarkers', coalesce((
      select pg_catalog.jsonb_agg(used.code order by used.code)
      from (
        select distinct material.tag_color as code
        from current_materials as material
        where material.tag_color is not null
      ) as used
    ), '[]'::jsonb),
    'usedGeo', coalesce((
      select pg_catalog.jsonb_agg(used.code order by used.code)
