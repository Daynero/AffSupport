-- Feature 024 — a search result says its note.
--
-- A file found by a word of its note looked like a file found by nothing: the note was searched
-- but not shown. `search_materials` returns each item's `note`, and the result card shows it.
-- Forward-only; ROLLBACK.md re-applies 20260916210000's search.
