...
-- ---------------------------------------------------------------------------------------------
-- A source whose Drive version moved without its bytes moving.
--
-- Sharing a file by link (022 does it to every catalog's video) moves its Drive version and nothing
-- else, and the catalog does not always hear of it. A process then binds the live version at start
-- and finalize compares it with the stale row — SOURCE_CHANGED for a file nobody changed. drive-ops
-- has just proved the file live; when its checksum is the one on record, the row takes the live
-- version. Different bytes are left for the catalog to reconcile.
