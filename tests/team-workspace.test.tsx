...
describe('guided team space workspace', () => {
  // A whole walkthrough — open, preview, settings, invite, back — takes longer than one
  // assertion's worth of patience on a slow machine.
  it(
    'enters a space content-first and keeps management behind space settings',
    { timeout: 20_000 },
    async () => {
      const team = makeTeam();
      ...
      // One press chooses a file, two open it — as a folder of files behaves everywhere.
      await user.dblClick(screen.getByRole('button', { name: 'Open launch.mp4' }));
