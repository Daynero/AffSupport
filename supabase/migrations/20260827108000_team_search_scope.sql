-- 011 (T060): search narrows to the open folder and to explorer row kinds.
--
-- The whole function is restated (a default-parameter overload would be
-- ambiguous for existing five-argument callers, so the old signature is
-- dropped first). Only the signature, two validations and two predicates
-- change; everything else is 20260815118000 verbatim.
-- Forward-only. Reverse steps are in ROLLBACK.md.

drop function if exists public.search_materials(uuid, text, jsonb, integer, integer);
...
revoke all on function public.search_materials(uuid, text, jsonb, integer, integer, text, text[])
from public, anon, authenticated, service_role;
grant execute on function public.search_materials(uuid, text, jsonb, integer, integer, text, text[])
  to authenticated;
comment on function public.search_materials(uuid, text, jsonb, integer, integer, text, text[]) is
  'Caller-checked exact-team catalog search; transcript bodies and provider identifiers are excluded. 011: optional folder scope and row-kind filter.';
