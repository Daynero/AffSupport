...
справді завантажується; глибоке посилання `/compressor` не перетворилось на 404;
...
Щоб heartbeat запрацював, потрібен секрет із публічним ключем Supabase:

```bash
gh secret set SOTY_SMOKE_SUPABASE_KEY --repo Daynero/AffSupport
```
