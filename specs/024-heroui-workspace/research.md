...
| `tests/design-components.test.tsx`   | **Rewritten.** It pins literal class markers of the _legacy_ shim, which D5 deletes. Its assertions move onto the inventory's own markers.                                                                                                                                                                                                  |
...
## What building the foundation changed about these decisions

Four of the decisions above met the working tree and came back different. Recorded here rather
than edited in place, because the reasons are worth more than the tidiness.

### D4 revised — preflight is imported, and layered under everything this product owns

The plan was to import Tailwind without its reset. That turned out to be solving the problem
twice. Both resets — Tailwind's preflight and HeroUI's own base — land in `@layer base`, the
lowest rung of the ladder declared in `styles/index.css`, and this product's own reset sits above
them in `soty-legacy`. A rule in a higher layer wins regardless of specificity, so the library's
resets reach only what this product never said anything about. Excluding them would have cost a
non-standard import path to prevent something the cascade already prevents.

### D4 again — the ladder has six rungs, not five, and the sixth is the point

`theme, base, soty-legacy, components, utilities, soty`.

The pre-024 stylesheets go **below** the library's components, not above. Left unlayered they
outrank every layer by construction, which would have meant every adopted component arriving
half-styled by rules written years before it — `button { padding: 0 }` and its hundred siblings
beating `.button--md`. And the new `soty` layer sits **above** the library, because a Soty
refinement has to be able to win without an `!important`.

### D16 (new) — twelve class names collided, and the collision was invisible until it was on screen

HeroUI's components carry generic base classes: `.button`, `.card`, `.checkbox`, `.spinner`,
`.tooltip`, `.toast`, `.skeleton`, `.pagination`, `.progress-bar`, `.empty-state`,
`.field-error`, `.toast-region`. This product uses all twelve, from before the 021 redesign
introduced the `ui-` prefix. Two stylesheets, one name, and whichever layer wins applies its base
to both — which is how the first build put the home page's call to action on screen as a yellow
square.

Twelve names, 57 usages in components and 121 selectors in CSS. All of them renamed with a
`soty-` prefix in one mechanical pass. Nothing moved a pixel, and the boundary is now visible in
the source: `.button` is the library's, `.soty-button` is a debt that dies with the screen that
holds it.

The general lesson is worth keeping: a component library with unprefixed class names shares a
namespace with everything the product has ever written, and the overlap is not discoverable by
reading either side. It took a screenshot.

### D7 revised — the library's stylesheet is imported whole

`@heroui/styles` can be imported per component; the package exports every component's CSS
separately, and its own index warns that the import order between them matters for specificity.
That is a list which must stay in step with forty-five adapters and respect a documented ordering
constraint — the kind of thing that is correct the day it is written and wrong a month later.

Imported whole it is **38.3 kB gzipped**, against 22 kB gzipped of team-only CSS this feature
deletes outright and a large share of a 91.5 kB global sheet. The first measurement, with the
library's CSS in and not one component adopted, put the largest chunk at 113.2 kB against the
baseline's 91.0 kB. That number is expected to come down, not up, as the screens migrate — and
T143 states it either way.

### D6 revised — the CSS gate needed no change at all

The audit predicted the token checker would collapse under generated CSS. It does not, because
Tailwind v4's Vite plugin never writes a stylesheet into the source tree: the authored file holds
`@import`, `@layer`, `@theme inline` and `var()` references, and nothing else. The gate passes
untouched and its exemption list is still empty.

The danger the audit named is real, though — it just arrives through TSX. That is what
