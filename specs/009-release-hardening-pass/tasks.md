...
**Tests**: Test tasks are included throughout, because this feature *is* about test coverage and verification. They are not optional here.
...
`A1`–`A18`, `B1`–`B14`, `C1`–`C21`, `D1`–`D12`, `E1`–`E9`, `F1`–`F15` refer to entries in [findings.md](./findings.md). Anything labelled there as *inferred* or *hypothesis* must be reproduced before it is fixed — see T001.
...
- [ ] T150 [P] [US5] Create `tests/pairing-verify-before-adopt.test.ts` asserting a planted token is discarded without ever reaching persistent storage (C12)
...
- [ ] T166 [US5] Create `apps/agent/src/server/tickets.ts` issuing path- and method-bound capability tickets with a five-minute lifetime, derived from but not equal to the session token
- [ ] T167 [US5] Issue tickets in the authenticated responses describing image, preview and media resources in `apps/agent/src/compressor/routes.ts`, `apps/agent/src/landing-preview/routes.ts` and `apps/agent/src/transcription/routes.ts`
...
- [ ] T175 [US5] Rebuild the ledger on boot from the durable tool state in `apps/agent/src/index.ts`, resolving and stat-ing each path, so restoration and authorisation cannot disagree
