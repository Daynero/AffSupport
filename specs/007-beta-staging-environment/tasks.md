...
- [x] T013 Make `validatePublicConfig` in `apps/web/src/lib/config.ts` environment-aware: parse `VITE_APP_ENVIRONMENT`, permit loopback `VITE_SITE_URL` / `VITE_SUPABASE_URL` in a production _build_ only when the environment is `beta`, and keep the existing rejection of loopback origins for `production`
