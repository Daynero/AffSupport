-- 011 follow-up: a scan that stopped must stop claiming progress.
--
-- Found in the beta run. `get_team_storage_health` reported `indexing` from the
-- connection's own state, so a queued job whose worker never ran left the chip
-- spinning "Indexing…" forever: no progress, no failure, nothing to act on. The
-- catalog is what proves a scan is alive — a job that has not been touched in
-- fifteen minutes is not slow, it is stopped, and someone who manages the space
-- can ask for another.
--
