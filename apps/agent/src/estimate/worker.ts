...
export class EstimationWorker {
  ...
  constructor(private jobs: () => CompressionJob[], private update: (id: string, patch: EstimatePatch, event: AgentEventType) => void, private compressionRunning: () => boolean, private cache = new EstimateCache()) {}
  ...
  private async estimate(job: CompressionJob, generation: number) {
    let temp = '';
    try {
      const source = await stat(job.inputPath); const key = estimateCacheKey(job.inputPath, source.size, source.mtimeMs, job.preset); const cached = this.cache.get(key);
      ...
      for (let i = 0; i < plan.length; i++) {
        ...
        const sample = plan[i], output = path.join(temp, `sample-${i}.h264`); const result = await this.runFfmpeg(buildEstimateArgs(job.inputPath, output, job.preset, sample.start, sample.duration));
        if (this.paused || generation !== this.generation || this.compressionRunning()) throw new Cancelled();
