...
- [x] T119 [US1] Migrate `apps/web/src/team/workspace/SettingsDialog.tsx` and `SpaceSettings.tsx` — the sticky bar and the five tabs — onto Modal + Tabs
...
| D1  | Labelling          | LOW      | Phase 3 (the inventory) is labelled `[US5]` though it mostly serves US1.                                                                                                                                                                                                                                                                                                                        | Left as is: US5 is the story the inventory _proves_ (a new screen is cheap to build), and every screen group carries `[US1]`. Recorded here so the label is not read as a mistake.                                                                                    |
...
_before_ `styles.css`. A group's remaining work is to delete its own copies from
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| The agent row menu closed on the first arrow press    | Its blur handler asked whether focus was still inside the _row_, which stopped being true once the surface was portalled out of it |
